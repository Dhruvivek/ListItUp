...
const CARD_CLASS = "rounded-[12px] border border-[#232323] bg-[#141414]";
...
function CountTile({ label, value, valueColor }: { label: string; value: number; valueColor?: string }) {
  return (
    <div className={`${CARD_CLASS} p-4`}>
      <div className={WIDGET_TITLE_CLASS}>{label}</div>
...
function BreakdownWidget({
  ...
  return (
    <div className={`${CARD_CLASS} p-5`}>
      <div className={`${WIDGET_TITLE_CLASS} mb-4`}>{title}</div>
      {entries.length === 0 ? (
